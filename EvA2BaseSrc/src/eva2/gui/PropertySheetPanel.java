...
//    private StringBuffer            m_HelpText;
    ...
    public void propertyChange(PropertyChangeEvent evt) {
        if (TRACE)
            System.out.println("PropertySheetPanel.propertyChange() " + m_Target.getClass() + ": calling wasModified");
...
//	            ////////////////// refactored by MK
                ...
                if (value == null) {
                    ...
                    if (getterClass.indexOf("java.") != 0)
                        System.out.println("Warning: Property \"" + name + "\" has null initial value.  Skipping.");
                    continue;
                }
    ...
    synchronized boolean updateValue(int i, Object newValue) {
        ...
        if (tmpEdit.getClass() != m_Editors[i].getClass()) {
            ...
            if (tmpEdit instanceof GenericObjectEditor)
                ((GenericObjectEditor) tmpEdit).setClassType(m_Properties[i].getPropertyType());
...
//                System.out.println(" m_Values[i]: "+ ((Integer) m_Values[i]).intValue());
    ...
    synchronized boolean wasModified(int propIndex, Object value, boolean followDependencies) {
        ...
        for (int i = 0; i < m_Properties.length; i++) {
            ...
            if (TRACE)
                System.out.println("# cmp " + BeanInspector.toString(o) + "\n# vs. " + BeanInspector.toString(m_Values[i]));
