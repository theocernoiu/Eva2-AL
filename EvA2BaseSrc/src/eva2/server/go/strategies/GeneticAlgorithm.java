...
public class GeneticAlgorithm implements InterfaceOptimizer, java.io.Serializable {
    ...
    protected InterfaceSelection m_ParentSelection = new SelectTournament();
    protected InterfaceSelection m_PartnerSelection = new SelectTournament();
    protected boolean m_UseElitism = true;
    protected int m_Plague = 0;
    protected int m_NumberOfPartners = 1;
...
//        }
     ...
     */
    protected Population generateChildren() {
...
//                }
    ...
    public void optimizeImpl() {
        ...
        if (this.m_Plague > 0) {
            for (int i = 0; i < this.m_Plague; i++)
                if (this.m_Population.size() > 2) this.m_Population.remove(this.m_Population.getWorstEAIndividual());
