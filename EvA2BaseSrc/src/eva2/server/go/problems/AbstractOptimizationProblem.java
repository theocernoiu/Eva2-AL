...
//    		System.out.println("bef: " + indy.toString());
    ...
    public boolean isPotentialOptimum(AbstractEAIndividual orig, double epsilon, double mutationStepSize, int numOfFailures) {
        ...
        if (numOfFailures < 0)
            numOfFailures = 100 * AbstractEAIndividual.getDoublePositionShallow(this.m_Template).length; // scales the effort with the number of problem dimensions
...
//    		initPerturb = epsilonPhenoSpace/(2*(Mathematics.getAvgRange(((InterfaceDataTypeDouble)orig).getDoubleRange())));
            ...
            if (maxEvaluations < 0)
                maxEvaluations = 100 * AbstractEAIndividual.getDoublePositionShallow(prob.m_Template).length; // scales the effort with the number of problem dimensions
        } else {
        ...
        if (epsilonFitConv > 0)
            term = new CombinedTerminator(new PhenotypeConvergenceTerminator(epsilonFitConv, 10 * dim, true, true), term, false);
