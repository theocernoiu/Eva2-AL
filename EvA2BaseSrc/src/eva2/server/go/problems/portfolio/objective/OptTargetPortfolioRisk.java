 ...
 */
public class OptTargetPortfolioRisk implements InterfaceOptimizationObjective, InterfacePortfolioSelectionTarget, java.io.Serializable {

    private boolean m_NormalizeTarget = false;
//    private double      m_TargetValue       = 1.0;
//    private SelectedTag m_OptType;

    public OptTargetPortfolioRisk() {
//        Tag[] tag = new Tag[4];
...
//        this.m_OptType = new SelectedTag(0, tag);
    ...
    public OptTargetPortfolioRisk(OptTargetPortfolioRisk p) {
...
//            this.m_OptType = (SelectedTag)p.m_OptType.clone();
    ...
    public Object clone() {
        return (Object) new OptTargetPortfolioRisk(this);
    }
    ...
    public double evaluatePortfolio(InterfaceDataTypeDouble indy, TFPortfolioSelectionProblemInterface prob) {
        ...
        double[] retA, riskA;
        double[][] corrA = prob.getAssetCorrelation();
        ...
        for (int i = 0; i < w.length; i++) {
            for (int j = 0; j < w.length; j++) {
                result += w[i] * w[j] * riskA[i] * riskA[j] * corrA[i][j];
            }
        }
...
//        }
    ...
    private double[] getBoundaries(TFPortfolioSelectionProblemInterface prob) {
        ...
        for (int i = 0; i < riskA.length; i++) {
            if (result[1] < riskA[i]) result[1] = riskA[i];
        }
        result[1] = result[1];
        return result;
    }
 ...
 */
    ...
    public String globalInfo() {
        return "Try to minimize the risk.";
    }
