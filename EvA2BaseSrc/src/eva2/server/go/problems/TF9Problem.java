 ...
 */
public class TF9Problem extends TF1Problem implements java.io.Serializable {
    ...
    public TF9Problem(TF9Problem b) {
     ...
     */
    public Object clone() {
        return (Object) new TF9Problem(this);
    }
    ...
    public double[] doEvaluation(double[] x) {
        ...
        for (int i = 0; i < x.length - 1; i++) {
            result[0] += -10 * Math.exp(-0.2 * Math.sqrt(Math.pow(x[i], 2) + Math.pow(x[i + 1], 2)));
        }
        for (int i = 0; i < x.length; i++) {
            result[1] += Math.pow(Math.abs(x[i]), 2) + 5 * Math.sin(Math.pow(x[i], 3));
        }
