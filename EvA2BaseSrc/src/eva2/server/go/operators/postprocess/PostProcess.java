...
//			if (TRACE) System.out.println("*** before " + subPop.getBestEAIndividual().getStringRepresentation());
            ...
            switch (method) {
                case nelderMead:
                    stepsPerf += PostProcess.processWithNMS(subPop, prob, term, subPop.size() - 1);
                    break;
                case cmaES:
                    stepsPerf += PostProcess.processWithCMA(subPop, prob, term, subPop.size() - 1);
                    break;
            }
...
//			PostProcessInterim.outputResult((AbstractOptimizationProblem)goParams.getProblem(), outputPop, 0.01, System.out, 0, 2000, 20, goParams.getPostProcessSteps());
            ...
            if (listener != null)
                listener.println("Best after post process:" + ((outputPop.size() > nBestPop.size()) ? ("(first " + nBestPop.size() + " of " + outputPop.size() + ")") : ""));
