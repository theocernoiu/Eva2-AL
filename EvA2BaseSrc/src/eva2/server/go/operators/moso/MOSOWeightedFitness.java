...
public class MOSOWeightedFitness implements InterfaceMOSOConverter, java.io.Serializable {
    ...
    public MOSOWeightedFitness() {
        ...
        for (int i = 0; i < this.m_Weights.m_DoubleArray.length; i++)
            this.m_Weights.m_DoubleArray[i] = 1 / ((double) this.m_Weights.m_DoubleArray.length);
    }
    ...
    public void setOutputDimension(int dim) {
        ...
        for (int i = 0; (i < this.m_Weights.m_DoubleArray.length) && (i < newWeights.length); i++)
            newWeights[i] = this.m_Weights.m_DoubleArray[i];
