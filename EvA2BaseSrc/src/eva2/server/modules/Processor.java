...
import eva2.server.go.strategies.GAD;
import eva2.server.go.strategies.InterfaceOptimizer;
import eva2.server.go.strategies.PSOD;
...
//    transient private BufferedWriter		m_OutputFile = null;
    ...
    public void addListener(RemoteStateListener module) {
        if (TRACE)
            System.out.println("Processor: setting module as listener: " + ((module == null) ? "null" : module.toString()));
    ...
    protected Population optimize(String infoString) {
        ...
        if (m_Statistics instanceof AbstractStatistics && this.goParams.getOptimizer() instanceof PSOD) {
            ((AbstractStatistics) m_Statistics).partial_disagreements = 0;
            ((PSOD) this.goParams.getOptimizer()).partial_disagreements = 0;
            ((AbstractStatistics) m_Statistics).extreme_disagreements = 0;
            ((PSOD) this.goParams.getOptimizer()).extreme_disagreements = 0;
        }

        if (m_Statistics instanceof AbstractStatistics && this.goParams.getOptimizer() instanceof GAD) {
            ((AbstractStatistics) m_Statistics).partial_disagreements = 0;
            ((GAD) this.goParams.getOptimizer()).partial_disagreements = 0;
            ((AbstractStatistics) m_Statistics).extreme_disagreements = 0;
            ((GAD) this.goParams.getOptimizer()).extreme_disagreements = 0;
        }
        ...
        while (isOptRunning() && (runCounter < m_Statistics.getStatisticsParameter().getMultiRuns())) {
            ...
            //////////////// Default stats
            if (m_Statistics instanceof AbstractStatistics && this.goParams.getOptimizer() instanceof PSOD) {
                ((AbstractStatistics) m_Statistics).partial_disagreements = ((PSOD) this.goParams.getOptimizer()).partial_disagreements;
                ((AbstractStatistics) m_Statistics).extreme_disagreements = ((PSOD) this.goParams.getOptimizer()).extreme_disagreements;
            }

            if (m_Statistics instanceof AbstractStatistics && this.goParams.getOptimizer() instanceof GAD) {
                ((AbstractStatistics) m_Statistics).partial_disagreements = ((GAD) this.goParams.getOptimizer()).partial_disagreements;
                ((AbstractStatistics) m_Statistics).extreme_disagreements = ((GAD) this.goParams.getOptimizer()).extreme_disagreements;
            }
...
//    		System.out.println(getGOParams().getOptimizer().getPopulation().getFunctionCalls() + " " + getGOParams().getOptimizer().getPopulation().getBestFitness()[0]);
            ...
            if (this.goParams.getOptimizer() instanceof InterfaceAdditionalPopulationInformer)
                informerList.add(this.goParams.getOptimizer());
