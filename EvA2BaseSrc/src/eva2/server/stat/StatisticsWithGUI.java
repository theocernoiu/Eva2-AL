...
public class StatisticsWithGUI extends AbstractStatistics implements Serializable, InterfaceStatistics {
    ...
    private static final String dbURL = "jdbc:sqlserver://localhost:1433;databaseName=test;user=sa;password=Visma123@;";
    ...
    public synchronized void startOptPerformed(String infoString, int runNumber, Object goParams) {
        ...
        if (runNumber == 0) {
            ...
            if (writeToDB && (m_FitnessFrame != null) && (m_FitnessFrame[0] != null)) {
                PlotInterface p = m_FitnessFrame[0].getPlotter();
                if ((p != null) && p.isValid()) ((Plot) p).clearAll();
            }
        }
            ...
            if ((p != null) && p.isValid()) {
                ...
                if (writeToDB) {
                    ...
                    try {
                        ...
                        stmt.executeUpdate(experimentSql, Statement.RETURN_GENERATED_KEYS);
                        ...
                        ((Plot) p).getFunctionArea().exportToDB(lastKey, stBatch);
                        int[] count = stBatch.executeBatch();
                        conn.commit();
